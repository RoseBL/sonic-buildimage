...
#define CTC_CLI_FLEXE_M_STR "Flexe module"
...
#define CTC_CLI_GROUP_FLOW_POLICER_ID_VALUE "value"
...
#define CTC_CLI_SRV6_M_STR "SRv6 Module"
#define CTC_CLI_SC_OAM_M_STR "SC OAM Module"
...
#define CTC_CLI_APS_BRIDGE_ID_DESC     "Bridge ID value"
...
#define CTC_CLI_GET_UDF_DATA(udf_data, udf_mask, mask_valid) \
{\
    int32 ret1 = 0, ret2 =0;\
    ret1 = ctc_cmd_xstr2array(argv[index+1], udf_data.udf, CTC_ACL_UDF_BYTE_NUM, 2);\
    ret2 = mask_valid ? ctc_cmd_xstr2array(argv[index+2], udf_mask.udf, CTC_ACL_UDF_BYTE_NUM, 2) : 0;\
    if (ret1 || ret2) \
    {\
...
#define CTC_CLI_L3PDU_L3IF_ACTION_INDEX "L3if action index"
...
#define CTC_CLI_LINKAGG_MEM_NUM "<0-255>, member num"
...
#define CTC_CLI_QOS_QUEUE_SET(p_queue) \
do{\
    (p_queue)->queue_type = CTC_QUEUE_TYPE_NETWORK_EGRESS; \
    index = CTC_CLI_GET_ARGC_INDEX("service-id"); \
    if (0xFF != index) \
    { \
        CTC_CLI_GET_UINT16("service-id", (p_queue)->service_id, argv[index + 1]); \
        (p_queue)->queue_type = CTC_QUEUE_TYPE_SERVICE_INGRESS; \
        is_qvalid = 1; \
    } \
    index = CTC_CLI_GET_ARGC_INDEX("port"); \
    if (0xFF != index) \
    { \
        CTC_CLI_GET_UINT16("gport", (p_queue)->gport, argv[index + 1]); \
        is_qvalid = 1; \
    } \
    index = CTC_CLI_GET_ARGC_INDEX("queue-id"); \
    if (0xFF != index) \
    { \
        CTC_CLI_GET_UINT16("queue-id", (p_queue)->queue_id, argv[index+1]); \
    } \
    index = CTC_CLI_GET_ARGC_INDEX("reason-id"); \
    if (0xFF != index) \
    { \
        CTC_CLI_GET_UINT16("reason-id", (p_queue)->cpu_reason, argv[index + 1]); \
        (p_queue)->queue_type = CTC_QUEUE_TYPE_EXCP_CPU; \
        is_qvalid = 1; \
    } \
}while(0)
